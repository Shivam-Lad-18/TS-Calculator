"use strict";
let input_area_g = document.getElementById("Input_area");
let sub_area_g = document.getElementById("sub_input");
let keys = document.getElementById("keys");
let trigo_drop = document.getElementById("trigo_drop");
let func_drop = document.getElementById("func_drop");
let trigo_btns = document.querySelectorAll(".t-func");
let count_badge = document.getElementById("count_badge");
let history_div = document.getElementById("history_div");
let memory_div = document.getElementById("memory_div");
let memory_btns = document.getElementById("memory_btns");
let chanbool = false;
let f_exp = false;
let f_equal = false;
let f_yrootx = false;
...
function setActive(clickedButton, button) {
    ...
    const historyOffcanvas = document.getElementById('history_offcanvas');
    const memoryOffcanvas = document.getElementById('memory_offcanvas');
    if (historyOffcanvas && memoryOffcanvas) {
        if (button === 'history_offcanvas') {
            ...
            const histButtons = document.querySelectorAll(".hist");
            histButtons.forEach((button) => {
                button.classList.add("active");
            });
            // Separate loop for .memo buttons
            const memoButtons = document.querySelectorAll(".memo");
...
function getHistory() {
    ...
    for (let i = 0; i < keys.length; i++) {
        let key = keys[i]; // Assuming keys is an array of numbers
        const storedValue = sessionStorage.getItem(key.toString()); // Get the value from sessionStorage
        // Check if storedValue exists before parsing
        if (storedValue) {
            // Parse the stored JSON value
            let hist_ele = JSON.parse(storedValue);
            ...
            if (hist_ele) {
                ...
                (_a = newele.querySelector('div')) === null || _a === void 0 ? void 0 : _a.addEventListener('click', () => {
...
function buttonpress(e) {
    let temp = e.target;
    let val = temp.id.trim(); // Get clicked element's ID
...
function trigoDrop(e) {
    let ebtn = e.target;
    let btn2nd = document.getElementById("trig2nd");
    let btnhyp = document.getElementById("hyp");
    let temp = e.target;
    ...
    if (e.target instanceof HTMLElement && e.target.closest("#trig2nd")) {
        ...
        if (!isNaN(inputValue)) { // Check if the input is a valid number
            ...
            const func = TrigoFunctions[functionName];
